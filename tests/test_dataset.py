...
def test_load_ohlcv_dataset_sets_missing_symbol_from_universe(tmp_path):
    universe = tmp_path / "universe.csv"
    universe.write_text("symbol,name\n000001,Demo\n", encoding="utf-8")
    cache_dir = tmp_path / "cache"
    frame = pd.DataFrame(
...
def test_load_ohlcv_dataset_rejects_symbol_mismatch(tmp_path):
    universe = tmp_path / "universe.csv"
    universe.write_text("symbol,name\n000001,Demo\n", encoding="utf-8")
    cache_dir = tmp_path / "cache"
    frame = pd.DataFrame(
    ...
    try:
        load_ohlcv_dataset(cache_dir=cache_dir, universe_path=universe)
    except ValueError as exc:
        assert "mismatched symbols" in str(exc)
    else:
